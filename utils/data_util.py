...
def culter_hvs(futures_data):
    return culter_hv(futures_data[:,3],futures_data[:,6],futures_data[:,5],futures_data[:,1],futures_data[:,2],futures_data[:,12],futures_data[:,13])

hv = culter_hvs(futures_data) # (2861,)

futures_data = np.delete(futures_data,[5,6,1,2,12,13],axis=1) # (2861, 11)

#
futures_data = np.c_[futures_data , hv] # (2861, 12)
...
# variance = np.divide(np.sum(np.power((futures_data - mean_futures) , 2)) , len(futures_data))
# print(variance)